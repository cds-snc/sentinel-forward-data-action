...
def test_handle_log_input_file_and_input_data_not_provided():
    assert (
        handle_log(
            ...
            log_analytics_workspace_key="foo",
            log_type="foo",
        )
        is False
    )
...
def test_handle_log_log_analytics_workspace_id_not_provided():
    assert (
        handle_log(
            file_name="foo.txt",
            input_data="foo",
            log_analytics_workspace_id=False,
            log_analytics_workspace_key="foo",
            log_type="foo",
        )
        is False
...
def test_handle_log_log_analytics_workspace_key_not_provided():
    assert (
        handle_log(
            ...
            input_data="foo",
            log_analytics_workspace_id="foo",
            log_analytics_workspace_key=False,
            log_type="foo",
        )
        is False
    )
...
@patch("src.lib.forwarder.post_data")
def test_handle_log_input_file_and_input_data_provided(mock_post_data):
    mock_post_data.return_value = True
    assert (
        handle_log(
            file_name="tests/data/test_file.txt",
    ...
    # assert that the number of post_data calls are equal to the number of lines in the test file
    assert mock_post_data.call_count == (
        sum(1 for _ in open("tests/data/test_file.txt")) + 1
    )
